...
# Read the CSV file
OldBigMoe <- read.csv("Moeller_2024_Final_Season.csv")
...
ui <- fluidPage(
  ...
  tags$style(HTML("
    body {
      background-image: url('https://raw.githubusercontent.com/IDBach16/Moeller_2024/main/2024%20Moeller%20Projects/IMG_2517.jpg');
  ...
  # Title Page
  titlePanel("Moeller Hitter Analysis"),
  navbarPage(
    title = "Moeller Hitting Dashboard",
    
    tabPanel("Introduction",
             h3("Overview of Dashboard"),
             p("Powered by AWRE data, the Moeller Hitting Analysis Dashboard offers deep insights into pitcher performance.
    ...
    tabPanel("Righty vs Lefty - Season",
             sidebarPanel(
    ...
    tabPanel("Moeller Batter Success Attack Zone Visualization",
...
server <- function(input, output, session) {
  ...
  statsRHPData <- reactive({
    filteredData() %>%
      filter(PitcherHand == 'R') %>%
      summarize(BF = n_distinct(Inning, Pitcher),
  ...
  df <- readr::read_csv("Moeller_2024_Final_Season.csv")

###### Double Check if you ever change this, there  is a read.csv and a read_csv within the app######
...
custom_colors <- c("StrikeLooking" = "navy", "Strike_Swing_Miss" = "yellow", "StrikeFoul" = "navy", "StrikeInPlay" = "yellow", "Ball" = "navy")
...
ui <- fluidPage(
  ...
  # Title Page
  titlePanel("Moeller Hitter Analysis 2025"),
  navbarPage(
    ...
    tabPanel("Introduction",
             ...
             p("Powered by AWRE data, the Moeller Hitting Analysis Dashboard offers deep insights into hitter performance.
               ...
               and scout opposing defenses. The dashboard also includes dynamic visualizations of batted ball outcomes and contact types, 
               providing a comprehensive view of hitting strategies and results."),
             ...
             p("Details:"),
             p("- Select Hitter Team: Dropdown menu to select the pitcher team."),
             p("- Select Date and Hitter: Dynamic UI elements to filter and select specific game dates and pitchers."),
             p("- Game Summary Table: Displays dplyr::summarized game data, including pitch types, velocity, and usage percentages."),
             p("- Usage vs RHP/LHP Tables: Separate tables showing pitch usage and statistics against right-handed and left-handed Pitchers."),
             ...
             p("- Stats vs Right-Handed Hitters Table: dplyr::summarizes key performance metrics against right-handed hitters."),
             p("- Usage vs Left-Handed Hitters Table: Shows pitch usage and statistics against left-handed hitters"),
             p("- Stats vs Left-Handed Hitters Table: dplyr::summarizes key performance metrics against left-handed hitters."),
             ...
             p("Details:"),
             p("- Select Pitcher Team and Pitcher: Dropdowns to filter data by pitcher team and specific pitchers."),
             p("- Batted Ball Table: dplyr::summarizes the number of batted balls and extra-base hits for each pitch type."),
    ...
    tabPanel("Data Dictionary (Analytics Team)",
             h3(tags$u("Purpose:")),
    ...
    tabPanel("Game Summary",
             sidebarPanel(
               selectInput("batterTeam1", "Select Batter Team:", choices = unique(BigMoe$BatterTeam)),
               uiOutput("dateUI"),
               uiOutput("batterUI")
             ...
             mainPanel(
               ...
               tags$h3("Usage vs RHP"),
               tableOutput("usageRHPTable"),
               tags$h3("Stats vs RHP"),
               tableOutput("statsRHPTable"),
               tags$h3("Usage vs LHP"),
               tableOutput("usageLHPTable"),
               tags$h3("Stats vs LHP"),
    ...
    tabPanel("(Moeller Only) Pitch Count vs Pitch Type - Season",
             fluidRow(
               column(12,
                      sidebarPanel(
                        selectInput("batterTeamMoeller", "Select Batter Team:", choices = "Moeller"),
                        selectInput("batterMoeller", "Select Batter:", choices = unique(BigMoe$Batter)),
                        selectInput("pitchTypeMoeller", "Select Pitch Type:", choices = unique(BigMoe$PitchType))
                      )
               )
             ),
             fluidRow(
               column(4,
                      tableOutput("pitchUsageTableMoeller")
               ),
               column(10,
                      uiOutput("plotOutputsMoeller")
               )
             )
    ),
    
    tabPanel("(Opponent Pitch) Count vs Pitch Type - Season",
             fluidRow(
               column(12,
                      sidebarPanel(
                        selectInput("batterTeamOpponent", "Select Batter Team:", choices = unique(BigMoe %>% filter(BatterTeam != 'Moeller') %>% pull(BatterTeam))),
                        selectInput("batterOpponent", "Select Batter:", choices = unique(BigMoe %>% filter(BatterTeam != 'Moeller') %>% pull(Batter))),
    ...
    tabPanel("Combined | Righty vs Lefty - Season",
             sidebarPanel(
               selectInput("batterTeam3", "Select Batter Team:", choices = unique(BigMoe$BatterTeam)),
               selectInput("batter3", "Select Batter:", choices = unique(BigMoe$Batter))
             ),
             mainPanel(
               ...
               h4("Against vs Left-Handed Pitchers"),
               tableOutput("usageLTable"),
               h4("Stats vs Left-Handed Pitchers"),
    ...
    tabPanel("Batted Ball - Season",
             sidebarPanel(
               selectInput("batterTeam4", "Select Batter Team:", choices = unique(BigMoe$PitcherTeam)),
               selectInput("batter4", "Select Batter:", choices = NULL),
    ...
    tabPanel("Moeller Batter Success Attack Zone Visualization (Updated! 4/9)",
...
server <- function(input, output, session) {
  # Update the batter dropdown based on the selected team
  observe({
    updateSelectInput(session, "batter1", choices = unique(BigMoe %>% filter(BatterTeam == input$batterTeam1) %>% pull(Batter)))
    updateSelectInput(session, "batterMoeller", choices = unique(BigMoe %>% filter(BatterTeam == input$batterTeamMoeller) %>% pull(Batter)))
    updateSelectInput(session, "batterOpponent", choices = unique(BigMoe %>% filter(BatterTeam == input$batterTeamOpponent) %>% pull(Batter)))
    updateSelectInput(session, "batter3", choices = unique(BigMoe %>% filter(BatterTeam == input$batterTeam3) %>% pull(Batter)))
    updateSelectInput(session, "batter4", choices = unique(BigMoe %>% filter(BatterTeam == input$batterTeam4) %>% pull(Batter)))
  })
  
  filteredData <- reactive({
    BigMoe %>%
      dplyr::filter(BatterTeam %in% input$batterTeam1,
                    Batter %in% input$batter1,
  ...
  data_dictionary <- data.frame(
    variable_name = c("PitchNo", "Date", "Time", "PAofInning", "PAofInning.1", "Batter", 
                      "BatterHand", "BatterTeam", "Pitcher", "Pitcher Hand", "Top/Bottom", 
    ...
    description = c("Pitch Number", "Date of the game", "Time of the pitch", "Plate appearance of inning", 
                    "Duplicated plate appearance of inning", "Batter name", "Batter hand (R/L)", 
                    "Batter team name", "Pitcher name", "Pitcher hand (R/L)", "Top or Bottom of inning", 
  ...
  # Update batter choices based on selected team and date
  output$batterUI <- renderUI({
    req(input$date)  # Ensure date is selected before updating batter choices
    selectInput("batter", "Select Batter:", choices = unique(teamData() %>% filter(Date == input$date) %>% pull(Batter)))
  })
  
  filteredData <- reactive({
    req(input$date, input$batter)  # Ensure date and batter are selected before filtering data
    teamData() %>%
      filter(Date == input$date, Batter == input$batter)
  })
...
#TEST for the hitter team data####
  ...
  statsRHPData <- reactive({
    filteredData() %>%
      filter(PitcherHand == 'R') %>%
      dplyr::summarize(AB = n_distinct(Inning, Pitcher),
  ...
  output$veloPlot <- renderPlotly({
    ...
    pitch_colors <- setNames(colorRampPalette(brewer.pal(14, "Set1"))(num_colors), unique_pitch_types)
    #### Creation of the GGplot#################
    pvp_game_plot <- ggplot(data = data, aes(x = Inning, y = PitchVelo, color = PitchType, shape = PitchResult)) +
      geom_point(size = 3, alpha = 0.7) +  # Adjusted size for better appearance
      geom_smooth(se = FALSE, method = "loess", span = 0.3) +
      facet_wrap(~ Pitcher, scales = "free_x") +
      scale_color_manual(values = pitch_colors) +
      labs(
        title = "Pitch Velocity by Inning and Pitcher",
        x = "Inning",
      ...
      scale_x_continuous(breaks = seq(min(data$Inning), max(data$Inning), by = 1),  # Custom breaks for whole numbers
  ...
  ###############################
  output$plotOutputsMoeller <- renderUI({
    plots <- lapply(conditions, function(condition) {
      plotname <- paste0("plotMoeller_", condition$Balls, "_", condition$Strikes)
      plotOutput(plotname)
    })
  ...
  observe({
    for (condition in conditions) {
      local({
        ...
        plotname <- paste0("plotMoeller_", my_condition$Balls, "_", my_condition$Strikes)
        output[[plotname]] <- renderPlot({
          filtered_df <- BigMoe %>%
            filter(BatterTeam == input$batterTeamMoeller & Batter == input$batterMoeller & PitchType == input$pitchTypeMoeller) %>% 
          ...
          condition_results <- data.frame(
            Category = c("StrikeLooking", "Strike_Swing_Miss", "StrikeFoul", "StrikeInPlay", "Ball"),
  ...
  observe({
    for (condition in conditions) {
      local({
        ...
        output[[plotname]] <- renderPlot({
          filtered_df <- BigMoe %>%
            filter(BatterTeam == input$batterTeamOpponent & Batter == input$batterOpponent & PitchType == input$pitchTypeOpponent) %>% 
            filter(Balls == my_condition$Balls, Strikes == my_condition$Strikes)
  ...
  output$usageRTable <- renderTable({
    total_pitches_r <- BigMoe %>%
      filter(BatterTeam == input$batterTeam3 & Batter == input$batter3 & PitcherHand == 'R') %>%
      nrow()
    ...
    BigMoe %>%
      ...
      group_by(Pitch = PitchType) %>%
      dplyr::summarize(No. = n(),
                `Usage %` = round(n() / total_pitches_r, 3) * 100,
                `2K` = sum(Strikes == 2),
                `2K%` = round(sum(Strikes == 2) / n() * 100, 3),
                `Strk%` = round(sum(PitchResult %in% c("StrikeLooking", "StrikeInPlay", "Strike_Swing_Miss", "StrikeFoul"), na.rm = TRUE) / n() * 100, 3),
                `Whiff` = sum(PitchResult == "Strike_Swing_Miss"),
  ...
  output$statsVsRTable <- renderTable({
    ...
    filteredData %>%
      dplyr::summarize(BF = n_distinct(Inning, Pitcher),
                K = sum(AtBatResult == "StrikeOut"),
                DP = sum(AtBatResult == "DoublePlay"),
                FO = sum(AtBatResult == "FlyOut"),
                GB = sum(AtBatResult == "GroundOut"),
  ...
  output$usageLTable <- renderTable({
    total_pitches_l <- BigMoe %>%
      filter(BatterTeam == input$batterTeam3 & Batter == input$batter3 & PitcherHand == 'L') %>%
      nrow()
    ...
    BigMoe %>%
      ...
      group_by(Pitch = PitchType) %>%
      dplyr::summarize(No. = n(),
                `Usage %` = round(n() / total_pitches_l, 3) * 100,
                `2K` = sum(Strikes == 2),
                `2K%` = round(sum(Strikes == 2) / n() * 100, 3),
                `Strk%` = round(sum(PitchResult %in% c("StrikeLooking", "StrikeInPlay", "Strike_Swing_Miss", "StrikeFoul"), na.rm = TRUE) / n() * 100, 3),
                `Whiff` = sum(PitchResult == "Strike_Swing_Miss"),
  ...
  grouped_data <- filtered_data2 %>%
    group_by(BatterTeam, Batter, PitchType, AtBatResult) %>%
    dplyr::summarize(count = n()) %>%
    ungroup()
  
  count_grouped_data <- filtered_data2 %>%
    mutate(CountSituation = paste(Balls, Strikes, sep = "-")) %>%
    group_by(BatterTeam, Batter, CountSituation, AtBatResult) %>%
    dplyr::summarize(count = n()) %>%
    ungroup() %>%
    filter(count < 20) 
  ...
  output$battedBallPlot1 <- renderPlotly({
    req(input$batterTeam4, input$batter4, input$pitchType)
    
    filtered1_data1_velo <- grouped_data %>%
      filter(BatterTeam == input$batterTeam4, Batter == input$batter4, PitchType == input$pitchType)
  ...
  output$battedBallPlot2 <- renderPlotly({
    req(input$batterTeam4, input$batter4)
    
    filtered_data2_velo <- filtered_data2 %>%
      filter(BatterTeam == input$batterTeam4, Batter == input$batter4) %>% 
      filter(PitchVelo > 60)
  ...
  output$battedBallPlot4 <- renderPlotly({
    req(input$batterTeam4, input$batter4, input$atBatResult)
    
    filtered3_data1_velo <- count_grouped_data %>%
      filter(BatterTeam == input$batterTeam4, Batter == input$batter4, AtBatResult == input$atBatResult)
  ...
  df <- readr::read_csv("Moeller_2025_Final_Season.csv")
  
  df <- df %>% 
    mutate(batter_success = case_when(
      PitchResult == "Ball" ~ 1,
      PitchResult == "Strike Foul" & "Ball" > 2 ~ 1,
      PitchResult == "Strike In Play" ~ 1,
      AtBatResult == "1B" ~ 1,
      AtBatResult == "2B" ~ 1,
      AtBatResult == "3B" ~ 1,
      AtBatResult == "HR" ~ 1,
  ...
  # Pitcher Model
  batter_mod <- lm(batter_success ~ pitch_name_no + Strikes + PitchVelo + Location, data = df)
  
  # Add predictions to the dataframe
  df <- df %>% 
    mutate(pred_batter_success_rate = predict(batter_mod),
           pred_batter_success_rate = case_when(
             pred_batter_success_rate < 0 ~ 0,
             pred_batter_success_rate > 1 ~ 1,
             TRUE ~ pred_batter_success_rate))
  
  # Simple Grouping
  grouped_df <- df %>% 
    filter(BatterTeam == "Moeller") %>%
    #mutate(Count = paste0(Balls, "-", Strikes)) %>%
    group_by(Batter, PitchType, AttackZone, Count) %>%
    dplyr::summarise(Pitches = n(),
              SuccessRate = mean(pred_batter_success_rate, na.rm = TRUE)) %>%
    ungroup() %>%
    filter(Pitches >= 5)
  
  
  # Update the selectInput choices
  updateSelectInput(session, "selected_batter", choices = unique(grouped_df$Batter))
  ...
  moeller_strike_zone_tile <- function(df, option = "AVG") {
    ...
    color_plot <- data %>%
      ...
      scale_y_continuous("",
                         limits = c(0.5, 4.6)) + 
      labs(title = paste0("Moeller Hitting ", tools::toTitleCase(option), " on Pitches"),
  ...
  filtered_data <- reactive({
    df %>%
      filter(Batter == input$selected_batter, 
             PitchType == input$selected_pitchtype)
 #            PitcherHand == input$selected_Hand)
  })
